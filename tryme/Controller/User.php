	...
	class UserController {
		function addUser($User){
			...
			try{
				...
				$stmt = $db->prepare("INSERT INTO user (ID, name, email, pass, adresse) VALUES (DEFAULT, :name, :email, :pass, :adresse)");
		...
		function UpdateUser($User){
			...
			try{
				      // prepare sql and bind parameters
				$sql = "UPDATE `user` SET `Name` = '".$User->getname()."', `email` = '".$User->getemail()."', `pass` = '".$User->getpass()."', `adresse` = '".$User->getadresse()."' ' WHERE `User`.`ID` = '".$User->getID()."'";
				$stmt = $db->prepare($sql);	
